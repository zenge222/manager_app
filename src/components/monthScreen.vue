<template>
  <div class="monthScreen has-header">
    ...
    <!-- <van-popup v-model="showSelDate" position="bottom">
      <van-datetime-picker
        title="请选择日期"
        @cancel="showSelDate=false"
        v-model="currentDate"
        :min-date="minDate"
        :max-date="maxDate"
        @confirm="submitDate"
...
import { _getScreenMonthDays } from "../service/appointment";
  ...
  methods: {
    ...
    initTime() {
     let date = new Date();
      ...
      // 月列表 monthsData
      let currentDays = m == 2 ? 28 : 30;
      let arr = [];
      for (let i = 1; i <= 31; i++) {
        let obj = { title: `${i}号`, val: i, checked: false };
        arr.push(obj);
      }
      this.monthsData = arr;
    },
    ...
    selWeek(item, index) {
      this.monthsData[index].checked = !this.monthsData[index].checked;
    },
    toScreenDay() {
      let _this = this;
      if (!this.sCurrentDate) {
        ...
        this.monthsData.forEach(val => {
          if (val.checked) {
            monthsList.push(val.val);
          }
        });
        monthsList = monthsList.join(",");
        let params = {
        ...
        _getScreenMonthDays(params).then(res => {
          if (res.data.success) {
            _this.isScreen = true;
